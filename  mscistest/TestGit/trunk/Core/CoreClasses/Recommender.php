...
";
    ...
    function getRecomendations(){
        ...
        foreach($this->Others as $users){
            ...
            foreach($users->getProducts() as $pO){
                if(in_array($pO,$this->User_A->getProducts()) || in_array($pO,$recomentationList)){
                    //do nothing;
                }
                else{
       ...
       if($counter<10){
            ...
            //complete with most popular bought items
            foreach($this->popular as $p){
                if(!in_array($p["SKU"],$this->User_A->getProducts()) && !in_array($p["SKU"],$recomentationList)){
                    $recomentationList[]=$p['SKU'];
               }
            }
        }
    ...
    function  get_popular_items(){
        $query="select product.SKU, count(product.SKU ) as rank from product,products_orders as pr where product.SKU=pr.SKU  group by product.SKU order by  count(product.SKU ) desc";
